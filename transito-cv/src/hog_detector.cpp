...
int main(int argc, char** argv) {
	try {
		...
		const double c_val = get_option(parser, "c", 150.0);
		...
		ofstream outfile;
		outfile.open(detector_name + "_train_result.txt");
		outfile << "precision, recall, average_precision" << std::endl;
		...
		// Create fHOG scanner
		typedef scan_fhog_pyramid<pyramid_down<5/*20*/> > image_scanner_type;
		...
		outfile << "training results: " << test_object_detection_function(detector, images_train, sign_boxes_train) << endl;
		outfile << "testing results:  " << test_object_detection_function(detector, images_test, sign_boxes_test) << endl;
		outfile.close();
