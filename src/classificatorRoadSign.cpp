...
int Shinik::ClassificatorRoadSign::Predict(const std::string & dir ) const {

	std::cout << "loading SVM from file\n";
	Ptr<SVM> svm = Algorithm::load<SVM>(dir + "/trained_svm");
	std::cout << "loaded\n";
	...
	std::ifstream file(dir + "/gt_test.csv");
	std::string line;
	std::vector<std::string> file_names;
	std::vector<int> labels;
	...
	while (std::getline(file, line)) {
		...
		const std::string file_name = dir + "/test/" + line.substr(0, found);
		file_names.push_back(file_name);
